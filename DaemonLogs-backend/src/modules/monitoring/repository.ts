...
export async function createMonitoring(data: { token: string; username?: string; usuario_id: number; is_valid: boolean }) {
...
export async function updateMonitoringUsername(id: number, username: string) {
  return prisma.contas_monitoramento.update({ where: { id }, data: { username } })
}
